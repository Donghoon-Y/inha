...
% (가정) 선정된 모터의 최대 소모 전류
motor_req_current = ceil(escSpecification); % 예: 30A

% 안전 계수 적용 (모터 최대 전류보다 10~20% 정도 여유를 두는 것이 좋음)
% 여유를 안 두려면 그냥 req_esc_current = motor_req_current;
margin = 1.2; 
req_esc_current = motor_req_current * margin;

% 2. 후보군 필터링 및 최저 무게 찾기
best_esc = {};         % 최종 선정될 ESC를 담을 변수
min_mass = inf;        % 초기 최소 무게를 무한대로 설정

for i = 1:size(esc_List, 1)
    esc_current = esc_List{i, 3}; % 3번째 열: 허용 전류(A)
    esc_mass    = esc_List{i, 4}; % 4번째 열: 무게(g)
    
    % 조건: ESC 허용 전류가 요구 전류보다 크거나 같은가?
    if esc_current >= req_esc_current
        
        % 조건: 현재까지 찾은 최소 무게보다 가벼운가?
        if esc_mass < min_mass
            min_mass = esc_mass;        % 최소 무게 업데이트
            best_esc = esc_List(i, :); % 해당 ESC 정보 저장
        end
